...
import com.hand.handtruck.ui.TruckInfo.CTruckFirstActivity;
...
public class CompanyTruckAdapter0 extends BaseExpandableListAdapter {
     private Context mContext;
    private List<FirstCompanyInfo> mList;
    private List<CarInfo> mcarlist;

    public CompanyTruckAdapter0(Context mContext, List<FirstCompanyInfo> mList, List<CarInfo> carlist) {
        this.mContext = mContext;
        this.mList = mList;
        this.mcarlist=carlist;
    }
    public void updateListView(List<FirstCompanyInfo> list, List<CarInfo> carlist){
        this.mList = list;
        this.mcarlist=carlist;
    ...
    public int getChildrenCount(int i) {
        ...
        if(i >= size){
            ...
            List<SecondCompanyInfo> list= mList.get(i).getChildren();
    ...
    public View getGroupView(int i, boolean b, View view, ViewGroup viewGroup) {
        ...
        if (view == null) {
            ...
            view = LayoutInflater.from(mContext).inflate(R.layout.lvitem_company_3, null);
            holder.catalog = (TextView) view.findViewById(R.id.catalog);
            view.setTag(holder);
        } else {
            holder = (GroupHolder) view.getTag();
        }
        holder.catalog.setText(mList.get(i).getName());
        return view;
    }
    ...
    public View getChildView(final int i, int i1, boolean b, View view, ViewGroup viewGroup) {
       ...
       final SecondCompanyInfo cb = mList.get(i).getChildren().get(i1);
        String remark=cb.getRemark();
        if("car".equals(remark)){
            ...
            holder.rl_2.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    cb1.setDeviceId(cb.getId()+"");
                    cb1.setParentName(mList.get(i).getName()+"");
                    cb1.setCarNumber(cb.getName());
                    Bundle bundle=new Bundle();
                    bundle.putSerializable("truckModel",(Serializable)cb1);
        ...
        }else if("company".equals(remark)){
            holder.rl_1.setVisibility(View.VISIBLE);
            holder.rl_2.setVisibility(View.GONE);
            holder.tv_company.setText(cb.getName());
            holder.tv_truck_number.setText(cb.getCountNumber()+"");
            holder.tv_online_truck_number.setText(cb.getSencondOnline()+"");
            holder.rl_1.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Bundle bund=new Bundle();
                    bund.putSerializable("first_info",(Serializable)cb);
                    //                bund.putSerializable("first_list",(Serializable)mcarlist);
                    //                Intent i=new Intent((Activity)mContext, CTruckFirstActivity.class);
                    //                i.putExtra("aa",bund);
                    //                mContext.startActivity(i);
                    CommonKitUtil.startActivity((Activity)mContext, CTruckFirstActivity.class, bund, false);
                }
            });
        }
        return view;
    ...
    class ChildHolder {
        TextView tv_company, tv_truck_number,tv_online_truck_number;
