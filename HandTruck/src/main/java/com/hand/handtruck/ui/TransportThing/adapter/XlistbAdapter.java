 ...
 * 
 */
public class XlistbAdapter extends BaseAdapter {

	private  List<TransportBean> mList;
	private Context context;
	private LayoutInflater inflater;

	public XlistbAdapter(Context context, List<TransportBean> list){
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView =inflater.inflate(R.layout.item_error_off, null);
		...
		if(item != null ){
			...
			String offDate=item.getUnloadEndTime();

			String onDate=item.getOnTime();

			viewHold.tv_close_time.setText(offDate+"");
			viewHold.tv_leave_address.setText(item.getUnloadAddress()+"");
