...
public class TransportErrorTask {
    private static Handler mHandler;
    private static Context mContext;
    private static TransportErrorTask instance = null;
    public TransportErrorTask() {
    }


    public static TransportErrorTask getInstance(Context context, Handler handler) {
        if (instance == null) {
            instance = new TransportErrorTask();
        }
        mContext = context;
        mHandler = handler;
        return instance;
    ...
    public void getTransportList(Map<String, String> map) {
        String url = Constants.HttpUrl.UN_LOAD;
    ...
    /*运输任务列表接口*/
    public void dopullListData(Map<String, String> map) {
        String url = Constants.HttpUrl.UN_LOAD;
        LogUtil.e("获取运输任务列表URL==" + url + map.toString());
        OkHttpUtils.post().url(url).params(map).build().execute(new StringCallback() {
            @Override
                ...
                if (!TextUtils.isEmpty(response)) {
                    ...
                    try {
                        ...
                        if("200".equals(code)) {
                            ...
                            List<TransportBean> message = companyResult.getContent();
                            if (message!=null && message.size() >0) {
                                mHandler.sendMessage(mHandler.obtainMessage(ConstantsCode.MSG_REQUEST_SUCCESS4, companyResult));
                            } else {
                                mHandler.sendEmptyMessage(ConstantsCode.MSG_REQUEST_EMPTY);
                            }
                        }else if("500".equals(code)){
